...
if [[ ${#REPOS[@]} -gt 0 ]]; then
    ...
    for REPO in "${REPOS[@]}"; do
        ...
        if [[ "${REPO}" =~ ^https?:\/\/.* ]] && [[ "${REPO}" == "https://copr.fedorainfracloud.org/coprs/"* ]]; then
          ...
          echo "Downloaded repo file ${REPO}"
        elif [[ ! "${REPO}" =~ ^https?:\/\/.* ]] && [[ "${REPO}" == *".repo" ]] && [[ -f "${CONFIG_DIRECTORY}/dnf/${REPO}" ]]; then
          cp "${CONFIG_DIRECTORY}/dnf/${REPO}" "/etc/yum.repos.d/${REPO##*/}"
        fi  
    done
fi
...
if [[ ${#INSTALL_PKGS[@]} -gt 0 ]]; then
  for i in "${!INSTALL_PKGS[@]}"; do
      ...
      if [[ "${PKG}" =~ ^https?:\/\/.* ]]; then
        ...
        INSTALL_PKGS[$i]="${CONFIG_DIRECTORY}/dnf/${PKG}"
        LOCAL_INSTALL=true
...
if [[ ${#INSTALL_PKGS[@]} -gt 0 && ${#REMOVE_PKGS[@]} -gt 0 ]]; then
    ...
    if ${CLASSIC_INSTALL} && ! ${HTTPS_INSTALL} && ! ${LOCAL_INSTALL}; then
      dnf override remove -y "${REMOVE_PKGS[@]}" $(printf -- "--install=%s " "${CLASSIC_PKGS[@]}")
    elif ${CLASSIC_INSTALL} && ${HTTPS_INSTALL} && ! ${LOCAL_INSTALL}; then
      dnf override remove -y "${REMOVE_PKGS[@]}" $(printf -- "--install=%s " "${CLASSIC_PKGS[@]}")
      ...
      dnf override remove -y "${REMOVE_PKGS[@]}"
      dnf install -y "${LOCAL_PKGS[@]}"
    elif ! ${CLASSIC_INSTALL} && ${HTTPS_INSTALL} && ! ${LOCAL_INSTALL}; then
      dnf override remove -y "${REMOVE_PKGS[@]}"
      dnf install -y "${HTTPS_PKGS[@]}"
...
if [[ ${#REPLACE[@]} -gt 0 ]]; then
    for REPLACEMENT in "${REPLACE[@]}"; do
        ...
        dnf override replace -y --experimental --from "repo=copr:copr.fedorainfracloud.org:${MAINTAINER}:${REPO_NAME}" ${REPLACE_STR}
