...
def negativo_repo_list []: nothing -> list<path> {
  try {
    ...
    | par-each {|repo|
      try {
        ^dnf -y repoinfo $repo --all --json | from json
      } catch {
        exit 1
      }
    }
...
def add_repos [$repos: list]: nothing -> list<string> {
  if ($repos | is-not-empty) {
    ...
    for $repo in $repos {
      ...
      try {
        ^dnf config-manager --add-repo=$repo --overwrite
      } catch {
        exit 1
      }
    }
  ...
  $repo_ids
    ...
    | try {
      ^dnf config-manager --setopt=<option>=$in
    } catch {
      exit 1
    }
...
def remove_repos [$repos: list]: nothing -> nothing {
  if ($repos | is-not-empty) {
    ...
    $repos
      | par-each {|repo|
        try {
          ^dnf repoinfo $repo --all --json | from json
        } catch {
          exit 1
        }
      }
...
def add_coprs [$copr_repos: list]: nothing -> list<string> {
  if ($copr_repos | is-not-empty) {
    ...
    for $copr in $copr_repos {
      ...
      try {
        ^dnf copr enable ($copr | check_copr)
      } catch {
        exit 1
      }
    }
...
def disable_coprs [$copr_repos: list]: nothing -> nothing {
  if ($copr_repos | is-not-empty) {
    ...
    for $copr in $copr_repos {
      ...
      try {
        ^dnf copr disable ($copr| check_copr)
      } catch {
        exit 1
      }
    }
...
def remove_pkgs [remove: record]: nothing -> nothing {
  ...
  if ($remove.packages | is-not-empty) {
    ...
    try {
      ^dnf remove -y ...($args) ...($remove.packages)
    } catch {
      exit 1
    }
  }
